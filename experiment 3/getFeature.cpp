...
void get_GLCMMoment(cv::Mat img, int angle, double GLCMMoment[5]) {
	int i, j;
	int height, width;
	height = img.rows;
	width = img.cols;
	...
	for (i = 0; i < height; i++) {
		for (j = 0; j < width; j++) {
			relative_value = img.at<cv::Vec3b>(i, j)[0];
...
{
	FILE* fp;
	cv::Mat Img;//cv::Mat图像类， Img图像的一个对象，相当于一个图像变量
...
void get_GaborFeature(cv::Mat img, double GaborFeature[80])//按格式输入图像
{
...
//提取颜色特征
void get_ColorFeature(cv::Mat img, double ColorFeature[Color_F_N])
{
	const int channels[] = { 0 };
	cv::Mat hist;//定义输出cv::Mat类型
	int dims = 1;//设置直方图维度
	const int histSize =  Color_F_N ; //直方图每一个维度划分的柱条的数目
	float pranges[] = { 0, 255 };//取值区间
	const float* ranges[] = { pranges };

	calcHist(&img, 1, channels, cv::Mat(), hist, dims, &histSize, ranges, true, false);//计算直方图
...
//HOG特征提取
std::vector<float> get_HOGFeature(cv::Mat img) 
{
