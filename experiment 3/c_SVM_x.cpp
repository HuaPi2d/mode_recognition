...
{
	//构建训练集和验证集，输入的训练数据每个类别有16个样本，每类取12个构建训练集，4个构建验证集
	float traindata_2D[20 * 12][20];//训练集
	float validatedata_2D[20 * 4][20];//验证集
...
void SVM_Classify(double traindata[], int trainLabel[], double testdata[], int classNum, int trainNum, int testNum, int featureNum, double P[][20])
{
	...
	svm->setKernel(SVM::LINEAR);//设置核类型为线性核, SVM::LINEAR，RBF
	svm->setGamma(0.01);//设置Gamma参数0.01
	//超参数优化，其实有两个超参数，本实验只针对一个参数C进行优化
	float minC = 0.5, maxC = 1.5, stepC = 0.1;//设置C参数得范围与步长
